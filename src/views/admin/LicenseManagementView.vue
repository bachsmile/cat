<template>
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    >
      <div>
        ...
        <p class="text-gray-500 mt-1">
          Cấp mới và quản lý danh sách mã license cho hệ thống npm
        </p>
      </div>
      <button
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div
        class="glass-panel p-6 rounded-3xl border relative overflow-hidden group"
      >
        <div
      ...
      >
        ...
        <h4 class="text-3xl font-bold mt-2 text-green-400">
          {{ activeCount }}
        </h4>
      </div>
      <div
      ...
      >
        ...
        <h4 class="text-3xl font-bold mt-2 text-amber-400">
          {{ expiringSoonCount }}
        </h4>
      </div>
    </div>
    ...
    <div class="glass-panel rounded-3xl border overflow-hidden">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left">
          <thead>
            <tr
              class="text-gray-500 text-xs uppercase tracking-widest border-b border-white/5"
            >
            ...
            >
              <td class="px-6 py-4">
                <div class="flex items-center gap-2">
                  <code
                    class="text-purple-400 font-mono text-sm bg-purple-500/10 px-2 py-1 rounded"
              ...
              <td class="px-6 py-4 text-right flex justify-end gap-2">
                <button
                  @click="toggleStatus(license)"
                  class="p-2 rounded-lg transition-colors"
                  :class="license.isActive ? 'hover:bg-amber-500/10 text-gray-400 hover:text-amber-400' : 'hover:bg-green-500/10 text-gray-400 hover:text-green-400'"
                  :title="license.isActive ? 'Vô hiệu hóa' : 'Kích hoạt'"
                >
                ...
                <button
                  @click="handleDelete(license.id)"
                  class="p-2 hover:bg-red-500/10 rounded-lg transition-colors text-gray-400 hover:text-red-400"
                  title="Xoá License"
                >
    ...
    >
      ...
      ></div>
      <div
        class="bg-[#0a0a0f] border border-white/10 rounded-3xl w-full max-w-md relative z-10 overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200"
      >
        <div
          class="p-6 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-purple-600/10 to-transparent"
        >
    ...
    >
      ...
      ></div>
      <div
        class="glass-panel rounded-3xl w-full max-w-sm relative z-10 overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200"
      >
        <div
          class="p-6 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-purple-600/10 to-transparent"
        >
        ...
        <div class="p-6 space-y-4">
          <div class="bg-white/5 p-4 rounded-2xl border border-white/5">
            ...
            <code class="text-purple-400 text-xs">{{
              selectedLicense?.licenseKey
            }}</code>
            <p
              class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-3 mb-1"
            >
              Cấu hình hiện tại
            </p>
            <p class="text-sm font-medium">
              Hết hạn:
              {{ selectedLicense ? formatDate(selectedLicense.expiresAt) : "" }}
            </p>
          </div>
          ...
          <div
            v-if="selectedOption === 'custom' || selectedOption === 'permanent'"
          >
            <label
              class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2"
            >
          ...
          <div
            v-else
            class="bg-purple-500/5 p-3 rounded-xl border border-purple-500/10 text-center"
          >
            <p class="text-xs text-purple-400 font-medium">
              Hết hạn mới:
              <span class="font-bold">{{ formatDate(renewDate) }}</span>
            </p>
          </div>
...
const toggleStatus = async (license: License) => {
  try {
    await licenseApi.update(license.id, { isActive: !license.isActive });
    await fetchLicenses();
  } catch (error) {
...
const handleDelete = async (id: string) => {
  if (!confirm("Bạn có chắc chắn muốn xoá license này không?")) return;
  try {
    await licenseApi.delete(id);
    await fetchLicenses();
  } catch (error) {
    console.error("Failed to delete license:", error);
    alert("Không thể xoá license. Vui lòng thử lại.");
  }
};
