<template>
  <div class="min-h-screen bg-[#050508] text-white flex items-center justify-center p-4">
    <div class="max-w-xl w-full">
      <!-- Success Icon Animation -->
      <div class="flex justify-center mb-8">
        <div class="w-24 h-24 bg-green-500/10 rounded-full flex items-center justify-center relative">
      ...
      <div class="text-center mb-12">
        <h1 class="text-3xl font-extrabold mb-4">{{ $t('npmPaymentSuccess') }}</h1>
      ...
      >
        <div v-if="licenseKey" class="glass-panel p-8 rounded-[2.5rem] border border-white/10 bg-white/[0.02] mb-8 text-center relative overflow-hidden animate-in zoom-in duration-500">
          <div class="absolute top-0 right-0 p-4 opacity-10">
            <KeyIcon class="w-20 h-20 text-purple-500" />
          </div>
          
          <p class="text-[10px] uppercase font-bold text-gray-500 tracking-widest mb-4">LICENSE CODE</p>
          <div class="flex items-center justify-center gap-3">
            <code class="text-2xl md:text-3xl font-mono font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-500 select-all">
              {{ licenseKey }}
            </code>
            <button 
              @click="copyKey"
        ...
        <div v-else class="text-center mb-8">
          ...
          >
            ...
            <GiftIcon v-else class="w-6 h-6 group-hover:rotate-12 transition-transform" />
            {{ claiming ? $t('commonLoading') : 'Claim License Code' }}
          </button>
          <p class="text-gray-500 text-xs mt-4 animate-pulse">Click to generate your premium license key</p>
        </div>
      </transition>

      <!-- Instructions -->
      <div v-if="licenseKey" class="glass-panel p-8 rounded-[2.5rem] border border-white/5 bg-black/40 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div class="space-y-6 text-sm text-gray-400">
          <div class="flex gap-4">
            <span class="w-6 h-6 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-[10px] shrink-0 font-bold">1</span>
            <p>{{ $t('npmStep1') }}</p>
          </div>
          <div class="flex gap-4">
            <span class="w-6 h-6 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-[10px] shrink-0 font-bold">2</span>
            <p>{{ $t('npmStep2') }}</p>
          </div>
...
app.<span class="text-yellow-400">use</span>(GlassUI, {
  licenseKey: <span class="text-emerald-500">"{{ licenseKey }}"</span>
          ...
          <div class="flex gap-4 pt-2">
             <span class="w-6 h-6 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-[10px] shrink-0 font-bold">3</span>
             <p>{{ $t('npmStep3') }}</p>
          </div>
        </div>
      </div>
    ...
    <div class="fixed top-0 left-0 w-full h-full -z-10 pointer-events-none opacity-20">
        <div class="absolute top-[20%] right-[10%] w-[30vw] h-[30vh] bg-purple-600 blur-[150px] rounded-full"></div>
        <div class="absolute bottom-[20%] left-[10%] w-[30vw] h-[30vh] bg-blue-600 blur-[150px] rounded-full"></div>
    </div>
  </div>
</template>
...
const claimCode = async () => {
  ...
  try {
    ...
    alert("System error: Could not generate license code. Please contact support.");
  } finally {
